...
- **mouse_click 在 64-bit Python 上独立运行时崩溃**：mouse_click/gui.py 缺少
  `CreateWindowExW` 和 `DefWindowProcW` 的 argtypes 设置，64-bit 下指针参数溢出。
  这是 mouse_click 项目自身的 bug（app_hotkey_manager 项目无此问题，其对所有 Win32 函数都设了
  argtypes）。**BindX 不受此影响**（hotkey_manager.py 已覆盖正确的 argtypes）。
  如需独立运行 mouse_click，建议使用 32-bit Python 或等上游项目补全 argtypes。
...
cd mouse_click
pip install -r requirements.txt    # keyboard, pynput
mouse_remapper.bat                 # 或 python app.py（需 32-bit Python，见已知限制）
```
